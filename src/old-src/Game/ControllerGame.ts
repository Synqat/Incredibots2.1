...
export class ControllerGame extends Controller {
  ...
  constructor() {
    ...
    this.m_fader.lineTo(window.innerWidth, 0)
    this.m_fader.lineTo(window.innerWidth, window.innerHeight)
    this.m_fader.lineTo(0, window.innerHeight)
    ...
    this.uneditableText.x = 0 + window.innerWidth / 2
    ...
    this.horizLineText.x = 0 + window.innerWidth / 2
    this.horizLineText.y = 102
    this.horizLineText.width = window.innerWidth
    ...
    this.vertLineText.x = 0 + window.innerWidth / 2
    ...
    this.shapeText.x = 0 + window.innerWidth / 2
    this.shapeText.y = 102
    this.shapeText.text =
      'Select a shape for the given condition.  You can use the arrow keys to scroll.'
  ...
  public Init(e: any): void {
    ...
    if (ControllerGameGlobals.loadedParts) {
      ...
      if (ControllerGameGlobals.justLoadedRobotWithChallenge) {
        this.allParts = this.allParts.concat(
          ControllerChallenge.challenge.allParts,
        )
    ...
    if (this.controllerType === 'challenge') {
      ControllerGameGlobals.minDensity =
        ControllerChallenge.challenge.minDensity == -Number.MAX_VALUE
          ? 1
          : ControllerChallenge.challenge.minDensity
      ControllerGameGlobals.maxDensity =
        ControllerChallenge.challenge.maxDensity == Number.MAX_VALUE
          ? 30
          : ControllerChallenge.challenge.maxDensity
      ControllerGameGlobals.maxRJStrength =
        ControllerChallenge.challenge.maxRJStrength == Number.MAX_VALUE
          ? 30
          : ControllerChallenge.challenge.maxRJStrength
      ControllerGameGlobals.maxRJSpeed =
        ControllerChallenge.challenge.maxRJSpeed == Number.MAX_VALUE
          ? 30
          : ControllerChallenge.challenge.maxRJSpeed
      ControllerGameGlobals.maxSJStrength =
        ControllerChallenge.challenge.maxSJStrength == Number.MAX_VALUE
          ? 30
          : ControllerChallenge.challenge.maxSJStrength
      ControllerGameGlobals.maxSJSpeed =
        ControllerChallenge.challenge.maxSJSpeed == Number.MAX_VALUE
          ? 30
          : ControllerChallenge.challenge.maxSJSpeed
      ControllerGameGlobals.maxThrusterStrength =
        ControllerChallenge.challenge.maxThrusterStrength == Number.MAX_VALUE
          ? 30
          : ControllerChallenge.challenge.maxThrusterStrength
    } else {
  ...
  public BuildBuildArea(): void {
    ...
    for (i = 0; i < this.NumBuildingAreas(); i++) {
      ...
      m_buildArea.beginTextureFill({
        texture: Gradient.getLinearGradientTexture(['#ff8f17', '#ffc150']),
        matrix: m,
        alpha: 0.2,
      })
      ...
      m = new Matrix()
      m_badBuildArea.beginTextureFill({
        texture: Gradient.getLinearGradientTexture(['#ff4d17', '#ff8f50']),
        matrix: m,
        alpha: 0.2,
      })
      ...
      m = new Matrix()
      m_selectedBuildArea.beginTextureFill({
        texture: Gradient.getLinearGradientTexture(['#ff4d17', '#ff8f50']),
        matrix: m,
        alpha: 0.2,
      })
  ...
  public Update(): void {
    ...
    if (!this.m_fader.visible) {
      ...
      if (ControllerGameGlobals.playingReplay && !this.simStarted)
        this.playButton(new MouseEvent(''))

      // update mouse position
      ControllerGameGlobals.mouseXWorldPhys =
        (Input.mouseX + this.draw.m_drawXOff) / this.m_physScale
      ControllerGameGlobals.mouseYWorldPhys =
        (Input.mouseY + this.draw.m_drawYOff) / this.m_physScale
      ...
      for (var i: number = 0; i < this.allParts.length; i++) {
        if (
          this.allParts[i] instanceof RevoluteJoint ||
          this.allParts[i] instanceof PrismaticJoint
        )
      ...
      if (!this.paused) {
        if (!ControllerGameGlobals.playingReplay) {
          if (this.frameCounter % ControllerGameGlobals.REPLAY_SYNC_FRAMES == 0)
            this.AddSyncPoint()
        ...
        if (
          this.frameCounter >= 9000 ||
          ControllerGameGlobals.cannonballs.length > 500
        )
          this.canSaveReplay = false
      }
    }
    ...
    if (!this.simStarted) {
      for (i = 0; i < this.m_buildAreas.length; i++) {
        ...
        if (this.hasZoomed || this.redrawBuildArea) {
          this.m_buildAreas[i].width =
            this.World2ScreenX(box.upperBound.x) -
            this.World2ScreenX(box.lowerBound.x)
          this.m_buildAreas[i].height =
            this.World2ScreenY(box.upperBound.y) -
            this.World2ScreenY(box.lowerBound.y)
          this.m_badBuildAreas[i].width =
            this.World2ScreenX(box.upperBound.x) -
            this.World2ScreenX(box.lowerBound.x)
          this.m_badBuildAreas[i].height =
            this.World2ScreenY(box.upperBound.y) -
            this.World2ScreenY(box.lowerBound.y)
        ...
        this.m_buildAreas[i].visible =
          !this.m_selectedBuildAreas[i].visible && this.partsFit
        this.m_badBuildAreas[i].visible =
          !this.m_selectedBuildAreas[i].visible && !this.partsFit
      }
    ...
    if (
      ...
      this.draw.DrawWorld(
        ...
        this.constructor.name === 'ControllerChallenge'
          ? ControllerChallenge.challenge
          : undefined,
      )
    ...
    if (!this.paused && this.ChallengeOver()) {
      ...
      if (
        !ControllerGameGlobals.playingReplay ||
        ControllerGameGlobals.viewingUnsavedReplay
      ) {
  ...
  public AddSyncPoint(): void {
    if (
      ...
      this.frameCounter != this.syncPoints[this.syncPoints.length - 1].frame
    ) {
      ...
      for (var i: number = 0; i < this.allParts.length; i++) {
        if (
          ...
          syncPoint.positions.push(
            Util.Vector(
              this.allParts[i].GetBody().GetPosition().x,
              this.allParts[i].GetBody().GetPosition().y,
            ),
          )
  ...
  private ComputeReplaySplines(type: number): Array<any> {
    ...
    for (
      var i: number = 0;
      i < ControllerGameGlobals.replay.syncPoints.length - 1;
      i++
    ) {
      h.push(
        ControllerGameGlobals.replay.syncPoints[i + 1].frame -
          ControllerGameGlobals.replay.syncPoints[i].frame,
      )
      var inner: Array<any> = new Array()
      for (
        var j: number = 0;
        j < ControllerGameGlobals.replay.syncPoints[0].positions.length;
        j++
      ) {
        if (type == 0)
          inner.push(
    ...
    for (i = 2; i < ControllerGameGlobals.replay.syncPoints.length - 1; i++) {
      ...
      for (
        ...
        inner.push(
          6 * (b[i][j] - b[i - 1][j]) - (h[i - 1] * v[i - 1][j]) / u[i - 1],
        )
      }
      v.push(inner)
    }
    ...
    for (i = 0; i < ControllerGameGlobals.replay.syncPoints.length - 1; i++) {
      ...
      for (
        ...
        deltaAngle =
          ...
          ControllerGameGlobals.replay.syncPoints[i].angles[j]
        if (Math.abs(deltaAngle) > 20) {
          a1 = Util.NormalizeAngle(
            ControllerGameGlobals.replay.syncPoints[i].angles[j],
          )
          a2 = Util.NormalizeAngle(
            ControllerGameGlobals.replay.syncPoints[i + 1].angles[j],
          )
          if (Math.abs(a1 - a2) < Math.PI) {
            deltaAngle = a2 - a1
  ...
  public SyncReplay(syncPoint: Object): void {
    ...
    for (var i: number = 0; i < this.allParts.length; i++) {
      if (
        ...
        this.allParts[i]
          .GetBody()
          .SetXForm(syncPoint.positions[curIndex], syncPoint.angles[curIndex])
        curIndex++
        bodiesUsed.push(this.allParts[i].GetBody())
      }
    }
    for (i = 0; i < ControllerGameGlobals.cannonballs.length; i++) {
      ControllerGameGlobals.cannonballs[i].SetXForm(
        syncPoint.cannonballPositions[i],
        0,
      )
    }
  }
  ...
  public SyncReplay2(syncPoint1: Object, syncPoint2: Object): void {
    var syncPointIndex: number =
      ControllerGameGlobals.replay.syncPoints.indexOf(syncPoint1)
    var bodiesUsed: Array<any> = new Array()
    var curIndex: number = 0
    for (var i: number = 0; i < this.allParts.length; i++) {
      if (
            ...
            (this.replaySplineXs[1][syncPointIndex][curIndex] +
              (this.frameCounter - syncPoint1.frame) *
                (this.replaySplineXs[2][syncPointIndex][curIndex] +
                  (this.frameCounter - syncPoint1.frame) *
                    this.replaySplineXs[3][syncPointIndex][curIndex]))
        var y: number =
            ...
            (this.replaySplineYs[1][syncPointIndex][curIndex] +
              (this.frameCounter - syncPoint1.frame) *
                (this.replaySplineYs[2][syncPointIndex][curIndex] +
                  (this.frameCounter - syncPoint1.frame) *
                    this.replaySplineYs[3][syncPointIndex][curIndex]))
        var angle: number =
            ...
            (this.replaySplineAngles[1][syncPointIndex][curIndex] +
              (this.frameCounter - syncPoint1.frame) *
                (this.replaySplineAngles[2][syncPointIndex][curIndex] +
                  (this.frameCounter - syncPoint1.frame) *
                    this.replaySplineAngles[3][syncPointIndex][curIndex]))
    ...
    for (i = 0; i < ControllerGameGlobals.cannonballs.length; i++) {
      if (syncPoint1.cannonballPositions.length > i) {
        ...
        var newX: number =
          (syncPoint1.cannonballPositions[i].x *
            (syncPoint2.frame - this.frameCounter) +
            syncPoint2.cannonballPositions[i].x *
              (this.frameCounter - syncPoint1.frame)) /
          frameDiff
        var newY: number =
          (syncPoint1.cannonballPositions[i].y *
            (syncPoint2.frame - this.frameCounter) +
            syncPoint2.cannonballPositions[i].y *
              (this.frameCounter - syncPoint1.frame)) /
          frameDiff
        ControllerGameGlobals.cannonballs[i].SetXForm(
          Util.Vector(newX, newY),
          0,
        )
      } else {
        ControllerGameGlobals.cannonballs[i].SetXForm(
          syncPoint2.cannonballPositions[i],
          0,
        )
      }
    }
  ...
  protected ChallengeOver(): boolean {
    throw new IllegalOperationError(
      'abstract ControllerGameGlobals.ChallengeOver() called',
    )
  }
  ...
  public GetScore(): number {
    throw new IllegalOperationError(
      'abstract ControllerGameGlobals.GetScore() called',
    )
  }
  ...
  protected GetBuildingArea() {
    throw new IllegalOperationError(
      'abstract ControllerGameGlobals.GetBuildingArea() called',
    )
  }
  ...
  protected CenterOnLoadedRobot(): void {
    ...
    for (var i: number = 0; i < this.allParts.length; i++) {
      if (
        this.allParts[i] instanceof ShapePart &&
        this.allParts[i].isCameraFocus
      )
        cameraPart = this.allParts[i]
    }
    if (!cameraPart) {
      for (i = 0; i < this.allParts.length; i++) {
      ...
      for (i = 0; i < this.allParts.length; i++) {
        if (this.allParts[i] instanceof ShapePart)
          this.allParts[i].SetCollisionGroup(-(i + 1))
      }
    ...
    if (cameraPart) {
      ...
      this.draw.m_drawXOff =
        cameraPart.centerX * this.m_physScale -
        ControllerGameGlobals.ZOOM_FOCUS_X
      this.draw.m_drawYOff =
        cameraPart.centerY * this.m_physScale -
        ControllerGameGlobals.ZOOM_FOCUS_Y
      if (oldX != this.draw.m_drawXOff || oldY != this.draw.m_drawYOff)
        this.hasPanned = true
    }
  }
  ...
  protected HandleCamera(): void {
    if (this.cameraPart) {
      ...
      var oldY: number = this.draw.m_drawYOff
      this.draw.m_drawXOff =
        this.cameraPart.GetBody().GetWorldCenter().x * this.m_physScale -
        ControllerGameGlobals.ZOOM_FOCUS_X
      this.draw.m_drawYOff =
        this.cameraPart.GetBody().GetWorldCenter().y * this.m_physScale -
        ControllerGameGlobals.ZOOM_FOCUS_Y
  ...
  public MouseDrag(): void {
    if (!this.simStarted) {
      ...
      if (
        ...
        !this.m_guiMenu.MouseOverMenu(
          ControllerGameGlobals.mouseXWorld,
          ControllerGameGlobals.mouseYWorld,
        ) &&
        (ControllerGameGlobals.mouseXWorld >= 120 ||
          !this.m_sidePanel.visible) &&
        ...
        if (
          ...
          !this.m_tutorialDialog.MouseOver(
            ControllerGameGlobals.mouseXWorld,
            ControllerGameGlobals.mouseYWorld,
          )
        ) {
          ...
          if (
            ...
            if (
              ...
              if (this.MouseOverSelectedPart()) {
                ...
                if (!Util.ObjectInArray(part, this.selectedParts)) {
                  ...
                  this.selectedParts = Util.RemoveFromArray(
                    part,
                    this.selectedParts,
                  )
                }
              ...
              for (var i: number = 0; i < this.selectedParts.length; i++) {
                this.draggingParts = Util.RemoveDuplicates(
                  this.draggingParts.concat(
                    this.selectedParts[i].GetAttachedParts(),
                  ),
                )
              }
              for (i = 0; i < this.draggingParts.length; i++) {
                if (
                  this.draggingParts[i] instanceof ShapePart ||
                  this.draggingParts[i] instanceof Thrusters
                ) {
                  this.draggingParts[i].dragXOff =
                    ControllerGameGlobals.mouseXWorldPhys -
                    this.draggingParts[i].centerX
                  this.draggingParts[i].dragYOff =
                    ControllerGameGlobals.mouseYWorldPhys -
                    this.draggingParts[i].centerY
            ...
            if (Input.isKeyDown(16) && ControllerGameGlobals.curRobotEditable) {
              ...
              this.draw.m_drawXOff -=
                ControllerGameGlobals.mouseXWorld -
                ControllerGameGlobals.prevMouseXWorld
              this.draw.m_drawYOff -=
                ControllerGameGlobals.mouseYWorld -
                ControllerGameGlobals.prevMouseYWorld
              if (oldX != this.draw.m_drawXOff || oldY != this.draw.m_drawYOff)
                this.hasPanned = true
            }
      ...
      if (this.draggingTutorial) {
        this.m_tutorialDialog.x =
          ControllerGameGlobals.mouseXWorld - this.initDragX
        this.m_tutorialDialog.y =
          ControllerGameGlobals.mouseYWorld - this.initDragY
      }
      ...
      if (
        ...
        this.m_conditionsDialog.FinishSelectingForCondition(
      ...
      if (this.curAction == ControllerGameGlobals.RESIZING_SHAPES) {
        ...
        var scaleFactor: number =
          ControllerGameGlobals.mouseXWorld - this.firstClickX
        if (scaleFactor >= 0) {
          scaleFactor /= 75
          scaleFactor += 1
        } else {
        ...
        for (i = 0; i < this.draggingParts.length; i++) {
          if (this.draggingParts[i] instanceof Circle) {
            if (
              this.draggingParts[i].initRadius * scaleFactor >
              Circle.MAX_RADIUS
            )
            ...
            if (length1 * scaleFactor > Triangle.MAX_SIDE_LENGTH)
              scaleFactor = Triangle.MAX_SIDE_LENGTH / length1
            if (length1 * scaleFactor < Triangle.MIN_SIDE_LENGTH)
              scaleFactor = Triangle.MIN_SIDE_LENGTH / length1
            ...
            if (length2 * scaleFactor > Triangle.MAX_SIDE_LENGTH)
              scaleFactor = Triangle.MAX_SIDE_LENGTH / length2
            if (length2 * scaleFactor < Triangle.MIN_SIDE_LENGTH)
              scaleFactor = Triangle.MIN_SIDE_LENGTH / length2
            ...
            if (length3 * scaleFactor > Triangle.MAX_SIDE_LENGTH)
              scaleFactor = Triangle.MAX_SIDE_LENGTH / length3
            if (length3 * scaleFactor < Triangle.MIN_SIDE_LENGTH)
              scaleFactor = Triangle.MIN_SIDE_LENGTH / length3
        ...
        for (i = 0; i < this.draggingParts.length; i++) {
          if (this.draggingParts[i] instanceof Circle) {
            ...
            this.draggingParts[i].centerY =
              this.initDragY + this.draggingParts[i].dragYOff * scaleFactor
            this.draggingParts[i].x1 =
              this.draggingParts[i].centerX +
              this.draggingParts[i].initX1 * scaleFactor
            this.draggingParts[i].y1 =
              this.draggingParts[i].centerY +
              this.draggingParts[i].initY1 * scaleFactor
            this.draggingParts[i].x2 =
              this.draggingParts[i].centerX +
              this.draggingParts[i].initX2 * scaleFactor
            this.draggingParts[i].y2 =
              this.draggingParts[i].centerY +
              this.draggingParts[i].initY2 * scaleFactor
            this.draggingParts[i].x3 =
              this.draggingParts[i].centerX +
              this.draggingParts[i].initX3 * scaleFactor
            this.draggingParts[i].y3 =
              this.draggingParts[i].centerY +
              this.draggingParts[i].initY3 * scaleFactor
          } else if (this.draggingParts[i] instanceof Cannon) {
            this.draggingParts[i].w = this.draggingParts[i].initW * scaleFactor
            this.draggingParts[i].Move(
              this.initDragX + this.draggingParts[i].dragXOff * scaleFactor,
            ...
            if (this.draggingParts[i] instanceof PrismaticJoint) {
              this.draggingParts[i].initLength =
                this.draggingParts[i].initInitLength * scaleFactor
            }
            ...
            this.draggingParts[i].x =
              this.initDragX +
              this.draggingParts[i].dragXOff * scaleFactor -
              this.draggingParts[i].w / 2
            this.draggingParts[i].y =
              this.initDragY +
              this.draggingParts[i].dragYOff * scaleFactor -
              this.draggingParts[i].h / 2
      ...
      if (
        ...
        if (
          this.candidateJointType == ControllerGameGlobals.NEW_REVOLUTE_JOINT
        ) {
          ...
          if (this.copiedJoint instanceof RevoluteJoint)
            rjoint.SetJointProperties(this.copiedJoint as RevoluteJoint)
          ...
          this.candidateJointType == ControllerGameGlobals.NEW_FIXED_JOINT
        ) {
          ...
          if (this.copiedJoint instanceof PrismaticJoint)
            pjoint.SetJointProperties(this.copiedJoint as PrismaticJoint)
      ...
      ((this.controllerType === 'sandbox' &&
        !(this.controllerType === 'challenge')) ||
        (this.controllerType === 'challenge' &&
          ControllerChallenge.challenge.mouseDragAllowed))
    ) {
      // mouse press
    ...
    if (minY < this.GetMinY()) {
      this.draw.m_drawYOff +=
        this.World2ScreenY(this.GetMinY()) - this.World2ScreenY(minY)
    } else if (maxY > this.GetMaxY()) {
      this.draw.m_drawYOff -=
        this.World2ScreenY(maxY) - this.World2ScreenY(this.GetMaxY())
    }

    var minX: number = this.Screen2WorldX(0)
    var maxX: number = this.Screen2WorldX(window.innerWidth)
    if (minX < this.GetMinX()) {
      this.draw.m_drawXOff +=
        this.World2ScreenX(this.GetMinX()) - this.World2ScreenX(minX)
      this.hasPanned = true
    } else if (maxX > this.GetMaxX()) {
      this.draw.m_drawXOff -=
        this.World2ScreenX(maxX) - this.World2ScreenX(this.GetMaxX())
      this.hasPanned = true
    }
  ...
  public keyInput(key: number, up: boolean): void {
    ...
    for (var i: number = 0; i < this.allParts.length; i++) {
      ...
      if (
        ...
        up &&
        !ControllerGameGlobals.playingReplay &&
        ((this.allParts[i] instanceof TextPart &&
          key == this.allParts[i].displayKey) ||
          (this.allParts[i] instanceof Cannon &&
            key == this.allParts[i].fireKey))
      ) {
        recorded = true
  ...
  public keyPress(key: number, up: boolean): void {
    ...
    if (
      !this.simStarted &&
      !this.m_sidePanel.EnteringInput() &&
      !this.m_fader.visible
    ) {
      if (up && key == 49) {
        this.circleButton()
      } else if (up && key == 50) {
    ...
    if (up && key == 27) {
      if (
        ...
        this.m_chooserWindow.visible &&
        !this.m_chooserWindow.fader.visible
      ) {
        this.m_chooserWindow.visible = false
        this.m_fader.visible = false
      }
      if (
        this.m_challengeWindow &&
        this.m_challengeWindow.visible &&
        !this.m_challengeWindow.fader.visible
      ) {
      ...
      if (
        ...
        !this.m_loginWindow.fader.visible
      ) {
  ...
  public mouseClick(up: boolean): void {
      ...
      if (
        ...
        this.selectedBuildArea =
          ControllerChallenge.challenge.buildAreas[
            ControllerChallenge.challenge.buildAreas.length - 1
          ]
    ...
    if (
      ...
      this.initDragX =
        ControllerGameGlobals.mouseXWorld - this.m_tutorialDialog.x
      this.initDragY =
        ControllerGameGlobals.mouseYWorld - this.m_tutorialDialog.y
      return
    }
    if (
      !ControllerGameGlobals.curRobotEditable ||
    ...
    if (
      ...
      for (i = 0; i < ControllerChallenge.challenge.buildAreas.length; i++) {
        if (
          ControllerGameGlobals.mouseXWorldPhys >
            ControllerChallenge.challenge.buildAreas[i].lowerBound.x &&
          ControllerGameGlobals.mouseXWorldPhys <
            ControllerChallenge.challenge.buildAreas[i].upperBound.x &&
          ControllerGameGlobals.mouseYWorldPhys >
            ControllerChallenge.challenge.buildAreas[i].lowerBound.y &&
          ControllerGameGlobals.mouseYWorldPhys <
    ...
    if (this.curAction == ControllerGameGlobals.NEW_CIRCLE) {
      ...
      if (!up && this.actionStep == 0) {
        ...
        if (
          ...
          var cannon: ShapePart = new Cannon(
            this.firstClickX,
            this.firstClickY,
            w,
          )
      ...
      if (!up && this.actionStep == 0) {
        ...
        if (
          ...
          var sideLen0: number = Util.GetDist(
            this.firstClickX,
            this.firstClickY,
            this.secondClickX,
            this.secondClickY,
      ...
      if (this.selectedParts.length == 0) {
        ...
        this.selectedParts.length == 1 &&
        this.selectedParts[0] instanceof Cannon
      ) {
        ...
        this.selectedParts.length == 1 &&
        this.selectedParts[0] instanceof Thrusters
      ) {
      ...
      if (
        ...
        var index1: number = -1,
          index2: number = -1
        for (i = 0; i < this.candidateJointParts.length; i++) {
          if (this.potentialJointPart1 == this.candidateJointParts[i])
            index1 = i
          if (this.potentialJointPart2 == this.candidateJointParts[i])
            index2 = i
        }
        if (
          index1 == this.candidateJointParts.length - 2 &&
          index2 == this.candidateJointParts.length - 1
        ) {
          index1 = 0
          index2 = 1
        } else {
      ...
      if (
        Math.abs(finalClickX - this.firstClickX) < 1 ||
        Math.abs(finalClickY - this.firstClickY) < 0.5
      ) {
      ...
      var text: TextPart = new TextPart(
        ...
        Math.max(this.firstClickX, finalClickX) -
          Math.min(this.firstClickX, finalClickX),
        Math.max(this.firstClickY, finalClickY) -
          Math.min(this.firstClickY, finalClickY),
        'New Text',
      )
  ...
  public CenterOnSelected(): void {
    if (this.selectedParts.length == 1) {
      ...
      var oldY: number = this.draw.m_drawYOff
      this.draw.m_drawXOff +=
        this.World2ScreenX(centerX) - ControllerGameGlobals.ZOOM_FOCUS_X
      this.draw.m_drawYOff +=
        this.World2ScreenY(centerY) - ControllerGameGlobals.ZOOM_FOCUS_Y
      if (oldX != this.draw.m_drawXOff || oldY != this.draw.m_drawYOff) {
        this.hasPanned = true
      }
    }
  ...
  public circleButton(): void {
    if (!ControllerGameGlobals.curRobotEditable || this.selectingCondition)
      return
    if (this.curAction == ControllerGameGlobals.NEW_CIRCLE) {
      this.curAction = -1
      this.redrawRobot = true
    ...
    if (!ControllerGameGlobals.adStarted) {
      ...
      if (
        ...
        if (!this.simStarted) {
          ...
          for (i = 0; i < this.allParts.length; i++) {
            if (this.allParts[i] instanceof PrismaticJoint)
              ControllerGameGlobals.collisionGroup *= 2
            if (
              this.allParts[i] instanceof JointPart ||
              this.allParts[i] instanceof Thrusters
            )
              this.allParts[i].Init(this.m_world)
          }
          ...
          this.savedDrawXOff = this.Screen2WorldX(
            ControllerGameGlobals.ZOOM_FOCUS_X,
          )
          this.savedDrawYOff = this.Screen2WorldY(
            ControllerGameGlobals.ZOOM_FOCUS_Y,
          )
        }
      } else {
        ...
        if (!this.partsFit)
          this.ShowDialog3(
            'You must fit your robot inside the starting box first!',
          )
        else if (tooManyShapes)
          this.ShowDialog3('Your robot contains too many shapes!  (Limit 500)')
        this.m_progressDialog.ShowOKButton()
        this.m_progressDialog.StopTimer()
      }
    }
  ...
  public pauseButton(): void {
    if (
      ControllerGameGlobals.playingReplay &&
      ControllerGameGlobals.replay.Update(this.frameCounter)
    )
  ...
  public resetButton(rateRobot: boolean = true): void {
    if (ControllerGameGlobals.playingReplay) {
      ...
      this.draw.m_drawXOff =
        this.savedDrawXOff * this.m_physScale -
        ControllerGameGlobals.ZOOM_FOCUS_X
      this.draw.m_drawYOff =
        this.savedDrawYOff * this.m_physScale -
        ControllerGameGlobals.ZOOM_FOCUS_Y
      this.hasPanned = true
      if (rateRobot) {
        if (
  ...
  public reportButton(): void {
    ...
    if (
      ...
      if (ControllerGameGlobals.userName == '_Public') {
        ...
        this.m_reportWindow = new ReportWindow(
          this,
          1,
          ControllerGameGlobals.curReplayID,
        )
  ...
  public rateWindowClosed(rating: number, redirect: number): void {
    ...
    if (
      ...
      Database.RateChallenge(
        ControllerGameGlobals.curChallengeID,
        rating,
        this.finishRatingChallenge,
      )
    } else {
      Database.RateRobot(
        ControllerGameGlobals.curRobotID,
        rating,
        this.finishRatingRobot,
      )
    }
  ...
  public restrictionsButton(): void {
    ...
    this.m_restrictionsDialog = new RestrictionsWindow(
      this as ControllerChallenge,
    )
  ...
  public sandboxSettingsButton(): void {
    ...
    if (!this.simStarted) {
      if (
        this.controllerType === 'sandbox' &&
        (!(this.controllerType === 'challenge') ||
          !ControllerChallenge.playOnlyMode)
      ) {
        this.m_sandboxWindow = new AdvancedSandboxWindow(
          this,
        ...
        this.m_fader.visible = true
        this.ShowDialog3(
          'That feature instanceof only available in Sandbox mode!',
        )
        this.m_progressDialog.ShowOKButton()
        this.m_progressDialog.StopTimer()
      }
      ...
      Main.BrowserRedirect(
        'http://www.incredibots.com/forums/posting.php?mode=reply&t=' +
          ...
          (ControllerGameGlobals.userName == '_Public'
            ? ''
            : '&sid=' + ControllerGameGlobals.sessionID),
        true,
      )
    }
  }
  ...
  public finishRatingRobot(): void {
    ...
    if (retVal) {
      ...
      this.m_progressDialog.SetMessage('Success!')
      this.m_progressDialog.HideInXSeconds(1)
      if (this.m_postReplayWindow && this.m_postReplayWindow.visible)
  ...
  public finishRatingReplay(): void {
    if (
      !Database.waitingForResponse ||
      Database.curTransactionType != Database.ACTION_RATE_REPLAY
    )
      return
  ...
  public finishRatingChallenge(): void {
    if (
      !Database.waitingForResponse ||
      Database.curTransactionType != Database.ACTION_RATE_CHALLENGE
    )
      return
    var retVal: boolean = Database.FinishRating()
    if (retVal) {
  ...
  public commentReplayButton(
    replayID: string = '',
    replayPublic: boolean = false,
  ): void {
    if (replayID == '') {
  ...
  public commentChallengeButton(
    challengeID: string = '',
    challengePublic: boolean = false,
  ): void {
    if (this.selectingCondition) return
    if (challengeID == '') {
  ...
  public rotateButton(): void {
    ...
    if (this.rotatingPart != null) {
      ...
      if (
        ...
        for (i = 0; i < this.selectedParts.length; i++) {
          this.rotatingParts = this.rotatingParts.concat(
            this.selectedParts[i].GetAttachedParts(),
          )
        }
      ...
      for (i = 0; i < this.rotatingParts.length; i++) {
        if (
          this.rotatingParts[i] instanceof ShapePart ||
          this.rotatingParts[i] instanceof Thrusters
        ) {
          ...
          if (this.rotatingParts[i] instanceof PrismaticJoint) {
            this.rotatingParts[i].rotateOrientation = Util.GetAngle(
              this.rotatingParts[i].axis,
            )
          }
        }
      }
    ...
    if (/*Main.premiumMode*/ true) {
      ...
      for (var i: number = 0; i < this.selectedParts.length; i++) {
        if (this.selectedParts[i] instanceof Circle) {
          ...
          var ca: Cannon = new Cannon(
            centerX -
              (this.selectedParts[i].x - centerX) -
              this.selectedParts[i].w,
            this.selectedParts[i].y,
            this.selectedParts[i].w,
          )
          ...
          var te: TextPart = new TextPart(
            this,
            centerX -
              (this.selectedParts[i].x + this.selectedParts[i].w / 2 - centerX),
            this.selectedParts[i].y,
            this.selectedParts[i].w,
            this.selectedParts[i].h,
          ...
          this.selectedParts[i] instanceof JointPart ||
          this.selectedParts[i] instanceof Thrusters
        ) {
          partMapping.push(-1)
        }
      }
      for (i = 0; i < this.selectedParts.length; i++) {
        ...
        if (this.selectedParts[i] instanceof FixedJoint) {
          ...
          var pj: PrismaticJoint = new PrismaticJoint(
            partMapping[index1],
            partMapping[index2],
            0,
            0,
          ...
          var axisAngle: number = Math.atan2(
            this.selectedParts[i].axis.y,
            this.selectedParts[i].axis.x,
          )
      ...
      for (var i: number = 0; i < this.selectedParts.length; i++) {
        if (this.selectedParts[i] instanceof Circle) {
          ...
          var ca: Cannon = new Cannon(
            this.selectedParts[i].x,
            centerY -
              (this.selectedParts[i].y - centerY) -
              this.selectedParts[i].w / 2,
            this.selectedParts[i].w,
          )
          ...
          var te: TextPart = new TextPart(
            ...
            this.selectedParts[i].x,
            centerY -
              (this.selectedParts[i].y + this.selectedParts[i].h / 2 - centerY),
            this.selectedParts[i].w,
            this.selectedParts[i].h,
            this.selectedParts[i].text,
            this.selectedParts[i].inFront,
  ...
  public multiDeleteButton(): void {
    ...
    for (var i: number = 0; i < this.selectedParts.length; i++) {
      if (this.selectedParts[i] instanceof ShapePart) {
        affectedJoints = affectedJoints.concat(
          this.selectedParts[i].GetActiveJoints(),
        )
        affectedJoints = affectedJoints.concat(
          this.selectedParts[i].GetActiveThrusters(),
        )
      }
      this.DeletePart(this.selectedParts[i], false, false)
    }
    if (this.selectedParts.length != 0) {
      var deletedParts: Array<any> = Util.RemoveDuplicates(
        this.selectedParts.concat(affectedJoints),
      )
  ...
  public densitySlider(value: number): void {
    if (
      ...
      if (oldDensity != value)
        this.AddAction(
          new ChangeSliderAction(
            this.selectedParts[0],
            ChangeSliderAction.DENSITY_TYPE,
            value - oldDensity,
          ),
        )
    }
  ...
  public densityText(value: string): void {
    if (this.lastSelectedShape instanceof ShapePart) {
      ...
      if (density < ControllerGameGlobals.minDensity)
        density = ControllerGameGlobals.minDensity
      if (density > ControllerGameGlobals.maxDensity)
  ...
  public strengthText(value: string): void {
    if (this.lastSelectedJoint instanceof JointPart) {
      ...
      if (this.lastSelectedJoint instanceof RevoluteJoint) {
        ...
        if (strength > ControllerGameGlobals.maxRJStrength)
          strength = ControllerGameGlobals.maxRJStrength
        if (isNaN(strength))
          strength = Math.min(15.0, ControllerGameGlobals.maxRJStrength)
      }
      if (this.lastSelectedJoint instanceof PrismaticJoint) {
        if (strength < 1.0) strength = 1.0
        if (strength > ControllerGameGlobals.maxSJStrength)
          strength = ControllerGameGlobals.maxSJStrength
        if (isNaN(strength))
          strength = Math.min(15.0, ControllerGameGlobals.maxSJStrength)
      }
      if (this.lastSelectedJoint instanceof RevoluteJoint)
      ...
      if (oldStrength != strength)
        this.AddAction(
          new ChangeSliderAction(
            this.lastSelectedJoint,
            ChangeSliderAction.STRENGTH_TYPE,
  ...
  public speedText(value: string): void {
    if (this.lastSelectedJoint instanceof JointPart) {
      ...
      if (this.lastSelectedJoint instanceof RevoluteJoint) {
        ...
        if (speed > ControllerGameGlobals.maxRJSpeed)
          speed = ControllerGameGlobals.maxRJSpeed
        if (isNaN(speed))
          speed = Math.min(15.0, ControllerGameGlobals.maxRJSpeed)
      }
      if (this.lastSelectedJoint instanceof PrismaticJoint) {
        if (speed < 1.0) speed = 1.0
        if (speed > ControllerGameGlobals.maxSJSpeed)
          speed = ControllerGameGlobals.maxSJSpeed
        if (isNaN(speed))
          speed = Math.min(15.0, ControllerGameGlobals.maxSJSpeed)
      }
      if (this.lastSelectedJoint instanceof RevoluteJoint)
        (this.lastSelectedJoint as RevoluteJoint).motorSpeed = speed
      if (this.lastSelectedJoint instanceof PrismaticJoint)
  ...
  public thrustText(value: string): void {
    if (this.lastSelectedThrusters instanceof Thrusters) {
      ...
      var strength: number = parseInt(value)
      if (strength < 1.0) strength = 1.0
      if (strength > ControllerGameGlobals.maxThrusterStrength)
        strength = ControllerGameGlobals.maxThrusterStrength
      if (isNaN(strength))
        strength = Math.min(15.0, ControllerGameGlobals.maxThrusterStrength)
  ...
  public cameraBox(value: boolean): void {
    if (
      ...
      this.AddAction(
        new CameraAction(this.selectedParts[0], value, oldCameraPart),
      )
    }
  }
  ...
  public collisionBox(value: boolean): void {
    if (
      ...
      ControllerGameGlobals.curRobotID = ''
      this.AddAction(
        new ShapeCheckboxAction(
          this.selectedParts[0],
          ShapeCheckboxAction.COLLIDE_TYPE,
          value,
        ),
      )
    } else if (
  ...
  public fixateBox(value: boolean): void {
    if (
      ...
      this.AddAction(
        new ShapeCheckboxAction(
          this.selectedParts[0],
          ShapeCheckboxAction.FIXATE_TYPE,
          value,
        ),
      )
    } else if (this.selectedParts.length > 1) {
  ...
  public outlineBox(value: boolean): void {
    if (
      ...
      this.AddAction(
        new ShapeCheckboxAction(
          this.selectedParts[0],
          ShapeCheckboxAction.OUTLINE_TYPE,
          value,
        ),
      )
    } else if (this.selectedParts.length > 1) {
  ...
  public terrainBox(value: boolean): void {
    if (
      ...
      this.AddAction(
        new ShapeCheckboxAction(
          this.selectedParts[0],
          ShapeCheckboxAction.TERRAIN_TYPE,
          value,
        ),
      )
    } else if (this.selectedParts.length > 1) {
  ...
  public undragableBox(value: boolean): void {
    if (
      ...
      this.AddAction(
        new ShapeCheckboxAction(
          this.selectedParts[0],
          ShapeCheckboxAction.UNDRAGABLE_TYPE,
          value,
        ),
      )
    } else if (this.selectedParts.length > 1) {
  ...
  public frontButton(): void {
    ...
    if (
      ...
      if (!this.selectedParts[0].inFront) {
        ...
        this.addChildAt(
          this.selectedParts[0].m_textField,
          this.getChildIndex(this.m_canvas) + 1,
        )
        this.AddAction(
          new MoveZAction(this.selectedParts[0], MoveZAction.FRONT_TYPE),
        )
      }
    }
    this.redrawRobot = true
  ...
  public backButton(): void {
    ...
    if (
      ...
      ControllerGameGlobals.curRobotID = ''
      this.AddAction(
        new MoveZAction(this.selectedParts[0], MoveZAction.BACK_TYPE, oldIndex),
      )
    } else if (
      this.selectedParts.length == 1 &&
      this.selectedParts[0] instanceof TextPart
      ...
      if (this.selectedParts[0].inFront) {
        ...
        this.addChildAt(
          this.selectedParts[0].m_textField,
          this.getChildIndex(this.m_canvas),
        )
        this.AddAction(
          new MoveZAction(this.selectedParts[0], MoveZAction.BACK_TYPE),
        )
      }
    }
    this.redrawRobot = true
  ...
  public scaleWithZoomBox(value: boolean): void {
    if (
      ...
      ControllerGameGlobals.curRobotID = ''
      this.AddAction(
        new TextCheckboxAction(
          this.selectedParts[0],
          TextCheckboxAction.SCALE_TYPE,
          value,
        ),
      )
    }
  ...
  public alwaysVisibleBox(value: boolean): void {
    if (
      ...
      this.AddAction(
        new TextCheckboxAction(
          this.selectedParts[0],
          TextCheckboxAction.DISPLAY_TYPE,
          value,
        ),
      )
    }
  ...
  public textKeyBox(input: GuiTextInput, keyNumber: number): void {
    if (input.enabled) {
      ...
      if (this.selectedParts[0] instanceof TextPart) {
        ...
        if (oldKey != keyNumber)
          this.AddAction(
            new ControlKeyAction(
              this.selectedParts[0],
              ControlKeyAction.TEXT_TYPE,
              oldKey,
              keyNumber,
            ),
          )
        ...
        if (
          ...
          this.AddAction(
            new ColourChangeAction(
              ...
              this.selectedParts[0] instanceof TextPart
                ? 0
                : this.selectedParts[0].opacity - oldOpacity,
            ),
          )
          ControllerGameGlobals.curRobotID = ''
          this.redrawRobot = true
      ...
      for (var i: number = 0; i < this.selectedParts.length; i++) {
        if (
          ...
          if (
            ...
            (!(this.selectedParts[i] instanceof TextPart) &&
              this.selectedParts[i].opacity != opacity)
          ) {
  ...
  public sizeText(target: GuiTextInput, value: string): void {
    if (this.lastSelectedText instanceof TextPart) {
      ...
      if (oldSize != this.lastSelectedText.size)
        this.AddAction(
          new TextSizeChangeAction(
            this.lastSelectedText,
            this.lastSelectedText.size - oldSize,
          ),
        )
    }
    this.m_sidePanel.TextAreaLostFocus()
  ...
  public minLimitText(text: string): void {
    ...
    if (this.lastSelectedJoint instanceof RevoluteJoint) {
      ...
      var oldLimit: number = (this.lastSelectedJoint as RevoluteJoint)
        .motorLowerLimit
      ;(this.lastSelectedJoint as RevoluteJoint).motorLowerLimit = limit
      if (limit != oldLimit)
        this.AddAction(
          new LimitChangeAction(
            this.lastSelectedJoint,
            LimitChangeAction.MIN_TYPE,
            oldLimit,
            limit,
          ),
        )
      ...
      if (this.selectedParts[0] instanceof RevoluteJoint && keyNumber != null) {
        ...
        if (oldKey != keyNumber)
          this.AddAction(
            new ControlKeyAction(
              this.selectedParts[0],
              ControlKeyAction.CONTRACT_TYPE,
              oldKey,
              keyNumber,
            ),
          )
      ...
      if (this.selectedParts[0] instanceof Thrusters && keyNumber != null) {
        ...
        if (oldKey != keyNumber)
          this.AddAction(
            new ControlKeyAction(
              this.selectedParts[0],
              ControlKeyAction.THRUSTERS_TYPE,
              oldKey,
              keyNumber,
            ),
          )
  ...
  public autoBox1(value: boolean): void {
    if (this.selectedParts[0] instanceof RevoluteJoint) {
      var sideEffect: boolean =
        value && (this.selectedParts[0] as RevoluteJoint).autoCCW
      ...
      this.AddAction(
        new JointCheckboxAction(
          this.selectedParts[0],
          JointCheckboxAction.AUTO_CW_TYPE,
          value,
          sideEffect,
        ),
      )
      ...
      this.AddAction(
        new JointCheckboxAction(
          this.selectedParts[0],
          JointCheckboxAction.AUTO_OSCILLATE_TYPE,
          value,
        ),
      )
    } else if (this.selectedParts[0] instanceof Thrusters) {
      ...
      this.AddAction(
        new JointCheckboxAction(
          this.selectedParts[0],
          JointCheckboxAction.AUTO_ON_TYPE,
          value,
        ),
      )
    }
  ...
  public autoBox2(value: boolean): void {
    if (this.selectedParts[0] instanceof RevoluteJoint) {
      var sideEffect: boolean =
        value && (this.selectedParts[0] as RevoluteJoint).autoCW
      ...
      this.AddAction(
        new JointCheckboxAction(
          this.selectedParts[0],
          JointCheckboxAction.AUTO_CCW_TYPE,
          value,
          sideEffect,
        ),
      )
  ...
  public rigidBox(value: boolean): void {
    ...
    this.AddAction(
      new JointCheckboxAction(
        this.selectedParts[0],
        JointCheckboxAction.RIGID_TYPE,
        value,
      ),
    )
  }
  ...
  public clearButton(): void {
    ...
    for (var i: number = 0; i < deletedParts.length; i++) {
      if (
        deletedParts[i] instanceof ShapePart ||
        deletedParts[i] instanceof TextPart
      ) {
        this.DeletePart(deletedParts[i], false)
      }
    }
    if (deletedParts.length != 0) {
      if (ControllerGameGlobals.curRobotEditable)
        this.AddAction(new ClearAction(deletedParts))
      this.CheckIfPartsFit()
      ControllerGameGlobals.curRobotID = ''
      this.redrawRobot = true
    ...
    if (!ControllerGameGlobals.curRobotEditable)
      ControllerGameGlobals.curRobotEditable = true
    if (this.m_sidePanel && this.m_sidePanel.visible)
      this.m_sidePanel.visible = false
  }
  ...
  public cutButton(): void {
    ...
    if (this.selectedParts.length == 1) {
      ...
      for (var i: number = 0; i < this.selectedParts.length; i++) {
        if (this.selectedParts[i] instanceof ShapePart) {
          var copiedPart: ShapePart = (
            this.selectedParts[i] as ShapePart
          ).MakeCopy()
          ...
          ControllerGameGlobals.clipboardParts.push(
            (this.selectedParts[i] as TextPart).MakeCopy(),
          )
          partMapping.push(-1)
          this.DeletePart(this.selectedParts[i], false, false)
        } else {
      ...
      for (i = 0; i < this.selectedParts.length; i++) {
        if (this.selectedParts[i] instanceof JointPart) {
          ...
          for (var j: number = 0; j < this.selectedParts.length; j++) {
            if (
              this.selectedParts[j] ==
              (this.selectedParts[i] as JointPart).part1
            )
              index1 = j
            if (
              this.selectedParts[j] ==
              (this.selectedParts[i] as JointPart).part2
            )
              index2 = j
          }
          if (index1 != -1 && index2 != -1)
            ControllerGameGlobals.clipboardParts.push(
              (this.selectedParts[i] as JointPart).MakeCopy(
                partMapping[index1],
                partMapping[index2],
              ),
            )
          ...
          for (j = 0; j < this.selectedParts.length; j++) {
            if (
              this.selectedParts[j] ==
              (this.selectedParts[i] as Thrusters).shape
            )
              index1 = j
          }
          if (index1 != -1)
            ControllerGameGlobals.clipboardParts.push(
              (this.selectedParts[i] as Thrusters).MakeCopy(
                partMapping[index1],
              ),
            )
          this.DeletePart(this.selectedParts[i], false, false)
  ...
  public pasteButton(): void {
    ...
    for (
      var i: number = 0;
      i < ControllerGameGlobals.clipboardParts.length;
      i++
    ) {
      if (ControllerGameGlobals.clipboardParts[i].isStatic) hasStatic = true
      if (ControllerGameGlobals.clipboardParts[i] instanceof Circle)
        hasCircles = true
      if (ControllerGameGlobals.clipboardParts[i] instanceof Rectangle)
        hasRects = true
      if (ControllerGameGlobals.clipboardParts[i] instanceof Triangle)
        hasTriangles = true
      if (ControllerGameGlobals.clipboardParts[i] instanceof FixedJoint)
        hasFJs = true
      if (ControllerGameGlobals.clipboardParts[i] instanceof RevoluteJoint)
        hasRJs = true
      if (ControllerGameGlobals.clipboardParts[i] instanceof PrismaticJoint)
        hasSJs = true
      if (ControllerGameGlobals.clipboardParts[i] instanceof Thrusters)
        hasThrusters = true
      if (ControllerGameGlobals.clipboardParts[i] instanceof Cannon)
        hasCannons = true
    }
    if (
      this.controllerType === 'challenge' &&
      ...
      this.m_fader.visible = true
      this.ShowDialog3(
        'Sorry, some of the copied parts are not allowed in this challenge!',
      )
      this.m_progressDialog.ShowOKButton()
      this.m_progressDialog.StopTimer()
      ...
      /*!Main.premiumMode && (hasThrusters || copiedThrusters)*/ false
    ) {
      this.m_fader.visible = true
    ...
    if (
      ...
      if (ControllerGameGlobals.clipboardParts[0] instanceof Cannon)
        this.m_sidePanel.ShowCannonPanel(
          ControllerGameGlobals.clipboardParts[0],
        )
      else
        this.m_sidePanel.ShowObjectPanel(
          ControllerGameGlobals.clipboardParts[0],
        )
      this.AddAction(new CreateAction(ControllerGameGlobals.clipboardParts[0]))
      ControllerGameGlobals.clipboardParts[0] =
        ControllerGameGlobals.clipboardParts[0].MakeCopy()
      this.curAction = ControllerGameGlobals.PASTE
      ControllerGameGlobals.curRobotID = ''
      ...
      for (i = 0; i < ControllerGameGlobals.clipboardParts.length; i++) {
        if (ControllerGameGlobals.clipboardParts[i] instanceof ShapePart) {
          if (
            ...
            ControllerGameGlobals.clipboardParts[i].GetMass() >
              (this.draggingPart as ShapePart).GetMass()
          )
            this.draggingPart = ControllerGameGlobals.clipboardParts[i]
        } else if (
          ControllerGameGlobals.clipboardParts[i] instanceof TextPart &&
          !this.draggingPart
        ) {
          this.draggingPart = ControllerGameGlobals.clipboardParts[i]
        }
      ...
      for (i = 0; i < ControllerGameGlobals.clipboardParts.length; i++) {
        if (ControllerGameGlobals.clipboardParts[i] instanceof ShapePart) {
          ControllerGameGlobals.clipboardParts[i].dragXOff =
            ...
            ControllerGameGlobals.clipboardParts[i].centerX
          ControllerGameGlobals.clipboardParts[i].dragYOff =
            (this.draggingPart as ShapePart).centerY -
            ControllerGameGlobals.clipboardParts[i].centerY
          this.allParts.push(ControllerGameGlobals.clipboardParts[i])
          this.selectedParts.push(ControllerGameGlobals.clipboardParts[i])
          this.draggingParts = Util.RemoveDuplicates(
            this.draggingParts.concat(
              ControllerGameGlobals.clipboardParts[i].GetAttachedParts(),
            ),
          )
          ControllerGameGlobals.clipboardParts[i] =
            ControllerGameGlobals.clipboardParts[i].MakeCopy()
          ...
          if (this.draggingPart instanceof ShapePart) {
            ControllerGameGlobals.clipboardParts[i].dragXOff =
              (this.draggingPart as ShapePart).centerX -
              ControllerGameGlobals.clipboardParts[i].x
            ControllerGameGlobals.clipboardParts[i].dragYOff =
              (this.draggingPart as ShapePart).centerY -
              ControllerGameGlobals.clipboardParts[i].y
          } else {
            ControllerGameGlobals.clipboardParts[i].dragXOff =
              (this.draggingPart as TextPart).x -
              ControllerGameGlobals.clipboardParts[i].x
            ControllerGameGlobals.clipboardParts[i].dragYOff =
              (this.draggingPart as TextPart).y -
              ControllerGameGlobals.clipboardParts[i].y
          }
          this.allParts.push(ControllerGameGlobals.clipboardParts[i])
          this.selectedParts.push(ControllerGameGlobals.clipboardParts[i])
          this.draggingParts = this.draggingParts.concat(
            ControllerGameGlobals.clipboardParts[i].GetAttachedParts(),
          )
          ControllerGameGlobals.clipboardParts[i] =
            ControllerGameGlobals.clipboardParts[i].MakeCopy()
      ...
      for (i = 0; i < ControllerGameGlobals.clipboardParts.length; i++) {
        if (ControllerGameGlobals.clipboardParts[i] instanceof JointPart) {
          ControllerGameGlobals.clipboardParts[i].dragXOff =
            (this.draggingPart as ShapePart).centerX -
            ControllerGameGlobals.clipboardParts[i].anchorX
          ControllerGameGlobals.clipboardParts[i].dragYOff =
            (this.draggingPart as ShapePart).centerY -
            ControllerGameGlobals.clipboardParts[i].anchorY
          this.allParts.push(ControllerGameGlobals.clipboardParts[i])
          this.selectedParts.push(ControllerGameGlobals.clipboardParts[i])
          var index1: number = -1,
            index2: number = -1
          for (var j: number = 0; j < oldClipboardParts.length; j++) {
            if (
              oldClipboardParts[j] ==
              (ControllerGameGlobals.clipboardParts[i] as JointPart).part1
            )
              index1 = j
            if (
              oldClipboardParts[j] ==
              (ControllerGameGlobals.clipboardParts[i] as JointPart).part2
            )
              index2 = j
          }
          ControllerGameGlobals.clipboardParts[i] =
          ...
          ControllerGameGlobals.clipboardParts[i] instanceof Thrusters
        ) {
          ControllerGameGlobals.clipboardParts[i].dragXOff =
            (this.draggingPart as ShapePart).centerX -
            ControllerGameGlobals.clipboardParts[i].centerX
          ...
          for (j = 0; j < oldClipboardParts.length; j++) {
            if (
              oldClipboardParts[j] ==
              (ControllerGameGlobals.clipboardParts[i] as Thrusters).shape
            )
              index1 = j
          }
          ControllerGameGlobals.clipboardParts[i] =
      ...
      if (this.m_chooserWindow.visible) {
        ...
        if (this.m_chooserWindow.dataType == SaveLoadWindow.HIGH_SCORE_TYPE) {
          Database.GetScoreData(
            ...
            success &&
              this.m_chooserWindow.m_scoreTypeBox &&
              this.m_chooserWindow.m_scoreTypeBox.selectedIndex == 2
              ? ''
              : Database.curSearch,
          ...
          this.m_chooserWindow.dataType == SaveLoadWindow.LOAD_ROBOT_TYPE
        ) {
          ...
          this.m_chooserWindow.dataType == SaveLoadWindow.LOAD_CHALLENGE_TYPE
        ) {
  ...
  public viewReplayButton(): void {
    if (ControllerGameGlobals.viewingUnsavedReplay) {
      ...
      ControllerGameGlobals.replayParts = this.allParts.filter(
        this.IsPartOfRobot,
      )
    ...
    if (Main.inIFrame) {
      ...
      if (
        ...
        if (this.m_scoreWindow && this.m_scoreWindow.visible)
          this.m_scoreWindow.ShowFader()
        this.ShowConfirmDialog(
          'Exit to main menu?     (Unsaved changes will be lost)',
          4,
        )
      }
    }
        ...
        if (/*!Main.premiumMode*/ false) {
          for (var i: number = 0; i < this.allParts.length; i++) {
            if (this.allParts[i] instanceof Thrusters) {
              this.m_fader.visible = true
              this.ShowDialog3(
                'Sorry, only supporters are allowed to save robots containing thrusters!',
              )
              this.m_progressDialog.ShowOKButton()
              this.m_progressDialog.StopTimer()
            ...
            if (this.allParts[i] instanceof Cannon) {
              this.m_fader.visible = true
              this.ShowDialog3(
                'Sorry, only supporters are allowed to save robots containing cannons!',
              )
              this.m_progressDialog.ShowOKButton()
              this.m_progressDialog.StopTimer()
  ...
  public finishSaving(): void {
    if (
      !Database.waitingForResponse ||
      Database.curTransactionType != Database.ACTION_SAVE_ROBOT
    )
      return
    ...
    if (robotID != '') {
      ...
      ControllerGameGlobals.ratedCurRobot = true
      ControllerGameGlobals.curRobotPublic =
        ControllerGameGlobals.potentialRobotPublic
      ControllerGameGlobals.curRobotFeatured =
        ControllerGameGlobals.potentialRobotFeatured
  ...
  public finishGettingSaveRobotData(): void {
    if (
      !Database.waitingForResponse ||
      Database.curTransactionType != Database.ACTION_GET_ROBOT_DATA
    )
      return
    if (Database.FinishGettingRobotData()) {
      this.m_progressDialog.visible = false
      if (this.m_chooserWindow) this.removeChild(this.m_chooserWindow)
      this.m_chooserWindow = new SaveLoadWindow(
        this,
        SaveLoadWindow.SAVE_ROBOT_TYPE,
      )
  ...
  public finishGettingLoadRobotData(): void {
    ...
    if (Database.FinishGettingRobotData()) {
      ...
      this.m_chooserWindow = new SaveLoadWindow(
        this,
        SaveLoadWindow.LOAD_ROBOT_TYPE,
      )
      this.addChild(this.m_chooserWindow)
    }
  }
  ...
  public finishLoading(): void {
    if (
      !Database.waitingForResponse ||
      Database.curTransactionType != Database.ACTION_LOAD_ROBOT
    )
      return
    ...
    if (robot) {
      ...
      if (ControllerGameGlobals.loadAndInsert) {
        ...
        if (
          ...
          ControllerGameGlobals.loadAndInsert = true
          this.ShowDialog3(
            'Sorry, that robot contains parts that are not allowed in this challenge!',
          )
          this.m_progressDialog.ShowOKButton()
          this.m_progressDialog.StopTimer()
        ...
        ControllerGameGlobals.curRobotID =
          ControllerGameGlobals.potentialRobotID
        ...
        if (robot.challenge) {
          ...
          ControllerGameGlobals.curChallengeID =
            ControllerGameGlobals.potentialChallengeID
  ...
  public finishDeleting(): void {
    ...
    if (id != '') {
      if (ControllerGameGlobals.curRobotID == id)
        ControllerGameGlobals.curRobotID = ''
    ...
    if (Main.inIFrame) {
      ...
      if (this.canSaveReplay) {
        ...
        if (this.frameCounter >= 9000)
          this.ShowDialog3(
            'Sorry, you can only save replays that are under 5 minutes in length.',
          )
        else
          this.ShowDialog3(
            'Sorry, you can only save replays that contain 500 or fewer cannonballs.',
          )
        this.m_progressDialog.ShowOKButton()
        this.m_progressDialog.StopTimer()
      }
  ...
  public finishAddingUser(): void {
    if (
      !Database.waitingForResponse ||
      Database.curTransactionType != Database.ACTION_ADD_USER
    )
      return
    ...
    if (retVal != '') {
      ...
      if (!this.m_scoreWindow || !this.m_scoreWindow.visible)
        this.m_fader.visible = false
      Main.premiumMode = false
      ControllerGameGlobals.userName = retVal.substring(
        retVal.indexOf('user: ') + 6,
        retVal.indexOf('password: ') - 1,
      )
      ControllerGameGlobals.password = retVal.substring(
        retVal.indexOf('password: ') + 10,
        retVal.indexOf('session: ') - 1,
      )
      ControllerGameGlobals.sessionID = retVal.substr(
        retVal.indexOf('session: ') + 9,
      )
  ...
  public finishSavingReplay(): void {
    if (
      !Database.waitingForResponse ||
      Database.curTransactionType != Database.ACTION_SAVE_REPLAY
    )
      return
  ...
  public finishGettingSaveReplayData(): void {
    if (
      !Database.waitingForResponse ||
      Database.curTransactionType != Database.ACTION_GET_REPLAY_DATA
    )
      return
    if (Database.FinishGettingReplayData()) {
      ...
      this.m_chooserWindow = new SaveLoadWindow(
        this,
        SaveLoadWindow.SAVE_REPLAY_TYPE,
      )
      this.addChild(this.m_chooserWindow)
      this.m_fader.visible = true
    }
  ...
  public finishGettingLoadReplayData(): void {
    ...
    if (Database.FinishGettingReplayData()) {
      ...
      this.m_chooserWindow = new SaveLoadWindow(
        this,
        SaveLoadWindow.LOAD_REPLAY_TYPE,
      )
      this.addChild(this.m_chooserWindow)
      this.m_fader.visible = true
    }
  ...
  public async finishLoadingReplay(e): void {
    if (
      !Database.waitingForResponse ||
      Database.curTransactionType != Database.ACTION_LOAD_REPLAY
    )
      return
  ...
  public processLoadedReplay(replayAndRobot: Array<any>): void {
    if (replayAndRobot) {
      ...
      if (
        ...
        ControllerGameGlobals.curReplayPublic =
          ControllerGameGlobals.potentialReplayPublic
        ControllerGameGlobals.curReplayFeatured =
          ControllerGameGlobals.potentialReplayFeatured
        ControllerGameGlobals.curChallengeID = ''
        ControllerGameGlobals.curChallengePublic = false
        ControllerGameGlobals.curChallengeFeatured = false
      }
  ...
  public finishDeletingReplay(): void {
    if (
      !Database.waitingForResponse ||
      Database.curTransactionType != Database.ACTION_DELETE_REPLAY
    )
      return
  ...
  public finishSavingChallenge(): void {
    if (
      !Database.waitingForResponse ||
      Database.curTransactionType != Database.ACTION_SAVE_CHALLENGE
    )
      return
  ...
  public finishLoadingChallenge(): void {
    if (
      !Database.waitingForResponse ||
      Database.curTransactionType != Database.ACTION_LOAD_CHALLENGE
    )
      return
  ...
  public processLoadedChallenge(challenge: Challenge): void {
    if (challenge) {
      ...
      ControllerChallenge.playChallengeMode =
        !ControllerGameGlobals.potentialChallengeEditable
      ControllerChallenge.playOnlyMode =
        !ControllerGameGlobals.potentialChallengeEditable
      ...
      ControllerGameGlobals.ratedCurChallenge = false
      ControllerGameGlobals.curChallengePublic =
        ControllerGameGlobals.potentialChallengePublic
      ControllerGameGlobals.curChallengeFeatured =
        ControllerGameGlobals.potentialChallengeFeatured
      ControllerGameGlobals.curRobotID = ''
      ControllerGameGlobals.curReplayID = ''
  ...
  public finishGettingSaveChallengeData(): void {
    if (
      !Database.waitingForResponse ||
      Database.curTransactionType != Database.ACTION_GET_CHALLENGE_DATA
    )
      return
    if (Database.FinishGettingChallengeData()) {
      ...
      this.m_chooserWindow = new SaveLoadWindow(
        this,
        SaveLoadWindow.SAVE_CHALLENGE_TYPE,
      )
      this.addChild(this.m_chooserWindow)
      this.m_fader.visible = true
    }
  ...
  public finishGettingLoadChallengeData(): void {
    ...
    if (Database.FinishGettingChallengeData()) {
      ...
      this.m_chooserWindow = new SaveLoadWindow(
        this,
        SaveLoadWindow.LOAD_CHALLENGE_TYPE,
      )
      this.addChild(this.m_chooserWindow)
      this.m_fader.visible = true
    }
  ...
  public finishGettingScoreData(): void {
    if (
      !Database.waitingForResponse ||
      Database.curTransactionType != Database.ACTION_GET_SCORE_DATA
    )
      return
    if (Database.FinishGettingScoreData()) {
      ...
      this.m_chooserWindow = new SaveLoadWindow(
        this,
        SaveLoadWindow.HIGH_SCORE_TYPE,
      )
      this.addChild(this.m_chooserWindow)
      this.m_fader.visible = true
    }
  ...
  public ShowDisabledDialog(): void {
    this.m_fader.visible = true
    if (this.m_progressDialog) this.removeChild(this.m_progressDialog)
    this.m_progressDialog = new DialogWindow(
      this,
      'That feature has been disabled for this level.',
      true,
    )
    this.m_progressDialog.ShowOKButton()
    this.m_progressDialog.StopTimer()
  ...
  public ShowTutorialWindow(
    ...
    this.m_tutorialDialog = new TutorialWindow(
      this,
      x,
      y,
      phraseNum,
  ...
  public ShowLinkDialog(
    ...
    this.m_linkDialog = new LinkWindow(
      this,
      msg1,
      msg2,
      isEmbedReplay,
  ...
  public DialogOK(): void {
    ...
    if (this.m_newUserWindow && this.m_newUserWindow.visible)
      this.m_newUserWindow.HideFader()
    if (this.m_loginWindow && this.m_loginWindow.visible)
      this.m_loginWindow.HideFader()
    if (this.m_scoreWindow && this.m_scoreWindow.visible)
      this.m_scoreWindow.HideFader()
    if (this.m_restrictionsDialog && this.m_restrictionsDialog.visible)
      this.m_restrictionsDialog.HideFader()
    if (this.m_conditionsDialog && this.m_conditionsDialog.visible)
      this.m_conditionsDialog.HideFader()
  }
  ...
  public HideDialog(): void {
    ...
    this.m_progressDialog.visible = false
    if (ControllerGameGlobals.failedChallenge)
      this.resetButton(new MouseEvent(''))
        ...
        for (var i: number = 0; i < this.allParts.length; i++) {
          if (jointFound) {
            ...
            if (t.shape == part) {
              ...
              t.isEnabled = false
              if (clearFromSelected)
                this.selectedParts = Util.RemoveFromArray(t, this.selectedParts)
              if (i != this.allParts.length - 1)
                this.allParts[i] = this.allParts[i + 1]
            }
          }
  ...
  protected FindCenterOfRobot(): ShapePart {
    ...
    for (var i: number = 0; i < this.allParts.length; i++) {
      if (
        ...
        var attachedParts: Array<any> = this.allParts[i]
          .GetAttachedParts()
          .filter(this.PartIsShape)
  ...
  private Zoom(zoomIn: boolean): void {
    var oldScale: number = this.m_physScale
    var centerX: number =
      (ControllerGameGlobals.ZOOM_FOCUS_X + this.draw.m_drawXOff) /
      this.m_physScale
    var centerY: number =
      (ControllerGameGlobals.ZOOM_FOCUS_Y + this.draw.m_drawYOff) /
      this.m_physScale
    if (zoomIn) {
      this.m_physScale *= 4.0 / 3.0
      if (this.m_physScale > ControllerGameGlobals.MAX_ZOOM_VAL)
        this.m_physScale = ControllerGameGlobals.MAX_ZOOM_VAL
    } else {
      this.m_physScale *= 3.0 / 4.0
      if (this.m_physScale < ControllerGameGlobals.MIN_ZOOM_VAL)
        this.m_physScale = ControllerGameGlobals.MIN_ZOOM_VAL
    }
    this.draw.m_drawXOff =
      centerX * this.m_physScale - ControllerGameGlobals.ZOOM_FOCUS_X
    this.draw.m_drawYOff =
      centerY * this.m_physScale - ControllerGameGlobals.ZOOM_FOCUS_Y
  ...
  private MaybeFinishCreatingPrismaticJoint(): void {
    ...
    for (var i: number = this.allParts.length - 1; i >= 0; i--) {
      if (
        this.allParts[i] instanceof ShapePart &&
        this.allParts[i].isEditable &&
        this.allParts[i] != this.jointPart
      ) {
        var part: ShapePart = this.allParts[i]
        if (part.InsideShape(jointX, jointY, this.m_physScale)) {
          candidateParts.push(part)
  ...
  private GetBodyAtMouse(): b2Body {
    ...
    aabb.lowerBound.Set(
      ControllerGameGlobals.mouseXWorldPhys - 0.001,
      ControllerGameGlobals.mouseYWorldPhys - 0.001,
    )
    aabb.upperBound.Set(
      ControllerGameGlobals.mouseXWorldPhys + 0.001,
      ControllerGameGlobals.mouseYWorldPhys + 0.001,
    ...
    for (var i: number = 0; i < shapes.length; ++i) {
      if (
        ...
        var tShape = shapes[i]
        var inside: boolean = tShape.TestPoint(
          tShape.m_body.GetXForm(),
          mousePVec,
        )
        if (inside) {
          body = tShape.m_body
  ...
  private GetPartAtMouse(): Part {
    ...
    if (candidateParts.length == 1 || allPartsSelected || noPartsSelected)
    ...
    for (
      ...
      if (!Util.ObjectInArray(candidateParts[i], this.selectedParts))
        return candidateParts[i]
    }
    return null
  }
}
