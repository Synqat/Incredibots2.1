...
export class ControllerMainMenu extends Controller {
  private static LOGO_STRETCH_WIDTHS: Array<number> = [
    101.2, 101, 100.7, 100.5, 100.2, 100, 99.7, 99.4, 99.1, 98.9, 98.6, 98.4,
    98.2, 98, 97.8, 97.7, 97.5, 97.5, 97.4, 97.4, 97.4, 97.5, 97.6, 97.8, 98.1,
    98.3, 98.6, 99, 99.3, 99.6, 100, 100.3, 100.7, 101, 101.3, 101.6, 101.8,
    102, 102.1, 102.2, 102.3, 102.2, 102.2, 102.1, 102, 101.8, 101.6, 101.4,
  ]
  private static LOGO_STRETCH_HEIGHTS: Array<number> = [
    97.7, 97.6, 97.7, 97.7, 97.7, 97.8, 97.9, 98, 98.2, 98.4, 98.7, 99, 99.3,
    99.6, 100, 100.3, 100.6, 100.9, 101.2, 101.5, 101.7, 101.8, 101.9, 102, 102,
    101.9, 101.8, 101.7, 101.5, 101.3, 101.1, 100.8, 100.6, 100.3, 100.1, 99.8,
    99.5, 99.3, 99, 98.8, 98.5, 98.4, 98.2, 98, 97.9, 97.8, 97.8, 97.7,
  ]
  ...
  constructor(straightToLevelSelect: boolean = false) {
    ...
    graphics.beginTextureFill({
      texture: Resource.cLevelSelectBox1M,
      matrix: m,
    })
    ...
    m.translate(0, 260)
    graphics.beginTextureFill({
      texture: Resource.cLevelSelectBox2M,
      matrix: m,
    })
    ...
    m.translate(0, 449)
    graphics.beginTextureFill({
      texture: Resource.cLevelSelectOtherBoxM,
      matrix: m,
    })
    ...
    this.levelSelectGui.addChild(button)
    this.logInButton = new GuiButton(
      'Log In',
      675,
      -5,
      120,
    ...
    this.logOutButton = new GuiButton(
      'Log Out',
      675,
      -5,
      120,
  ...
  private DrawGroundOutlineCircle(
    xPos: number,
    yPos: number,
    radius: number,
  ): void {
    ...
    const texture = Gradient.getLinearGradientTexture(
      [Util.HexColourString(177, 102, 46), Util.HexColourString(171, 59, 34)],
      radius * 2,
    )
  ...
  private DrawGroundCircle(xPos: number, yPos: number, radius: number): void {
    ...
    matrix.translate(0, yPos)
    const texture = Gradient.getLinearGradientTexture(
      [Util.HexColourString(249, 172, 101), Util.HexColourString(240, 70, 45)],
      radius * 2,
    )
    this.sGround.beginTextureFill({ texture, matrix })
  ...
  public finishGettingLoadRobotData(e: Event): void {
    ...
    if (Database.FinishGettingRobotData(e)) {
      ...
      this.m_loadWindow = new SaveLoadWindow(
        this,
  ...
  public processLoadedRobot(robot: Robot): void {
    ...
    for (var i: number = 0; i < loadedParts.length; i++) {
      if (
        loadedParts[i] instanceof Thrusters ||
        loadedParts[i] instanceof Cannon
      )
        hasThrusters = true
    }
  ...
  public finishLoggingInGold(e: Event): void {
    if (
      !Database.waitingForResponse ||
      Database.curTransactionType != Database.ACTION_LOGIN_GOLD
    )
      return
  ...
  public ReloadLoadWindow(): void {
    ...
    if (
      this.m_loadWindow.dataType != SaveLoadWindow.HIGH_SCORE_TYPE &&
      this.m_loadWindow.yourRobotsBox.selected
    ) {
      if (this.m_loadWindow.dataType == SaveLoadWindow.LOAD_ROBOT_TYPE) {
        ...
        this.m_loadWindow.dataType == SaveLoadWindow.LOAD_REPLAY_TYPE
      ) {
        Database.GetReplayData(
          ControllerGameGlobals.userName,
          ControllerGameGlobals.password,
  ...
  public DialogOK(e: Event): void {
    ...
    if (this.m_goldLoginWindow && this.m_goldLoginWindow.visible)
      this.m_goldLoginWindow.HideFader()
    if (this.m_newUserWindow && this.m_newUserWindow.visible)
      this.m_newUserWindow.HideFader()
  }
  ...
  private contestButtonPressed(e: MouseEvent): void {
    Main.BrowserRedirect(
      'http://incredibots.com/forums/viewtopic.php?t=119105',
      true,
    )
  }
  ...
  private forumPostButtonPressed(e: MouseEvent): void {
    Main.BrowserRedirect(
      'http://incredibots.com/forums/viewtopic.php?p=1269792',
      true,
    )
  }
  ...
  public Update(): void {
    if (ControllerMainMenu.channel && !Main.enableSound) {
      ...
      var st: SoundTransform = new SoundTransform(
        ControllerGameGlobals.introVolume,
      )
    ...
    if (!this.levelSelectGui.visible) {
      this.sLogo.width =
        (this.sLogo.texture.width *
          ControllerMainMenu.LOGO_STRETCH_WIDTHS[this.logoFrame]) /
        100.0
      this.sLogo.height =
        (this.sLogo.texture.height *
          ControllerMainMenu.LOGO_STRETCH_HEIGHTS[this.logoFrame]) /
        100.0
  ...
  public SyncReplay(syncPoint: Object): void {
    ...
    for (var i: number = 0; i < this.allParts.length; i++) {
      if (
        ...
        this.allParts[i]
          ...
          .SetXForm(
            Util.Vector(
              syncPoint.positions[curIndex].x,
              syncPoint.positions[curIndex].y,
            ),
    ...
    for (i = 0; i < ControllerMainMenu.cannonballs.length; i++) {
      ControllerMainMenu.cannonballs[i].SetXForm(
  ...
  private ComputeReplaySplines(type: number): Array<any> {
    ...
    for (var i: number = 0; i < this.replay.syncPoints.length - 1; i++) {
      h.push(
        this.replay.syncPoints[i + 1].frame - this.replay.syncPoints[i].frame,
      )
      var inner: Array<any> = new Array()
      for (
        var j: number = 0;
        j < this.replay.syncPoints[0].positions.length;
        j++
      ) {
        if (type == 0)
          inner.push(
            (this.replay.syncPoints[i + 1].positions[j].x -
              this.replay.syncPoints[i].positions[j].x) /
              h[i],
          )
        else if (type == 1)
          inner.push(
            (this.replay.syncPoints[i + 1].positions[j].y -
              this.replay.syncPoints[i].positions[j].y) /
              h[i],
          )
        else {
          var deltaAngle: number =
            this.replay.syncPoints[i + 1].angles[j] -
